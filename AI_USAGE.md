...
I corrected this to sign `"<timestamp>.<body>"` and include the timestamp in the `X-Dodo-Signature` header as `t=<timestamp>,v1=<signature>`. This matches the Stripe webhook signing scheme, which includes the timestamp in the signed payload so replays older than 5 minutes are detectable.
