...
mod tests {
    ...
    async fn test_concurrent_pay_only_one_succeeds() {
        ...
        let cust_id = sqlx::query_scalar!(
            "INSERT INTO customers (business_id, name, email) VALUES ($1, 'Test', 'conc@test.com') RETURNING id",
            biz_id
        )
        .fetch_one(&pool).await.unwrap();
        ...
        // Fire 10 concurrent pay attempts via direct DB logic
        // (In a real integration test you'd hit the HTTP server)
        let n = 10;
        ...
        for i in 0..n {
            ...
            handles.push(tokio::spawn(async move {
                barrier.wait().await; // all goroutines start at same time

                let mut tx = pool.begin().await.unwrap();
                // Try to lock and update invoice atomically
                let row = sqlx::query!(
                ...
                sqlx::query!(
                    "INSERT INTO payment_attempts (invoice_id, idempotency_key, request_hash, card_token, status, psp_ref)
        ...
        let attempt_count = sqlx::query_scalar!(
            "SELECT COUNT(*) FROM payment_attempts WHERE invoice_id = $1", invoice_id
        )
    ...
    async fn test_idempotency_same_key_no_second_attempt() {
        ...
        let cust_id = sqlx::query_scalar!(
            "INSERT INTO customers (business_id, name, email) VALUES ($1, 'Test', 'idem@test.com') RETURNING id",
            biz_id
        )
        .fetch_one(&pool).await.unwrap();
        ...
        sqlx::query!(
            "INSERT INTO payment_attempts (invoice_id, idempotency_key, request_hash, card_token, status)
             VALUES ($1, $2, $3, 'tok_success', 'succeeded')",
            invoice_id, idem_key, request_hash
        )
        .execute(&pool).await.unwrap();

        // Second attempt with same key — should find existing, NOT insert
        ...
        assert!(existing.is_some(), "Idempotency key lookup must find existing attempt");
    ...
    // ─────────────────────────────────────────────────────────────────────
    // TEST 3: PSP timeout — invoice must NOT be stuck in bad state
    // ─────────────────────────────────────────────────────────────────────
    #[tokio::test]
    async fn test_psp_timeout_invoice_stays_open() {
        ...
        let cust_id = sqlx::query_scalar!(
            "INSERT INTO customers (business_id, name, email) VALUES ($1, 'Test', 'timeout@test.com') RETURNING id",
            biz_id
        )
        .fetch_one(&pool).await.unwrap();
        ...
        // Simulate what happens on PSP timeout:
        // 1. Payment attempt inserted as pending
        // 2. PSP times out → attempt marked failed with code "psp_timeout"
        // 3. Invoice STAYS open (not corrupted)

        let attempt_id = sqlx::query_scalar!(
            "INSERT INTO payment_attempts (invoice_id, idempotency_key, request_hash, card_token, status)
             VALUES ($1, 'timeout-idem-key', 'hash', 'tok_timeout', 'pending') RETURNING id",
            invoice_id
        )
        .fetch_one(&pool).await.unwrap();

        // Simulate timeout handler: update attempt to failed, leave invoice open
        sqlx::query!(
            "UPDATE payment_attempts SET status = 'failed', failure_code = 'psp_timeout', updated_at = NOW() WHERE id = $1",
            attempt_id
        )
        .execute(&pool).await.unwrap();
        ...
        let attempt_status = sqlx::query_scalar!(
            "SELECT status FROM payment_attempts WHERE id = $1", attempt_id
        )
        .fetch_one(&pool).await.unwrap();

        let failure_code = sqlx::query_scalar!(
            "SELECT failure_code FROM payment_attempts WHERE id = $1", attempt_id
        )
        .fetch_one(&pool).await.unwrap();

        assert_eq!(invoice_status, "open", "Invoice must remain open after PSP timeout");
        assert_eq!(attempt_status, "failed", "Attempt must be marked failed");
